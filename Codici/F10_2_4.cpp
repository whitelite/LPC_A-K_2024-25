...
double derivata(int s, double x){
...
int newton(double& x, double tau, int s){
    int cont = 0; double delta;
    do{
        delta = funzione(s, x)/derivata(s, x);        //questo delta equivale a x_i - x_(i-1), che ci consente di utilizzare il criterio d'arresto praticamente con 1 solo controllo (e della memoria occupata) e nessun operazione in più 
        x -= delta;
